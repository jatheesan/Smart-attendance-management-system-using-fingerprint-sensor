    ...
    <div class="col-lg-7">
        ...
        <select id="course_code" class="form-control @error('course_code') is-invalid @enderror" name="course_code">
            <option value="" style="display: none;"
                {{ old('course_code', optional($attendance3GStudent)->course_code ?: '') == '' ? 'selected' : '' }}
...
<div class="form-group row {{ $errors->has('date') ? 'has-error' : '' }}">
    ...
    <div class="col-lg-7">
        <input class="form-control" name="date" type="date" id="date"
            value="{{ old('date', optional($attendance3GStudent)->date) }}"
...
<div class="form-group row {{ $errors->has('hours') ? 'has-error' : '' }}">
    ...
    <div class="col-lg-7">
        <input class="form-control" name="hours" type="number" id="hours"
            value="{{ old('hours', optional($attendance3GStudent)->hours) }}"
...
<div class="form-group row {{ $errors->has('hall') ? 'has-error' : '' }}">
    ...
    <div class="col-lg-7">
        <select class="form-control" id="hall" name="hall">
            <option value="" style="display: none;"
                {{ old('hall', optional($attendance3GStudent)->hall ?: '') == '' ? 'selected' : '' }}
            ...
            @foreach(['CUL-1' => 'CUL-1',
                ...
                <option value="{{ $key }}"
                    {{ old('hall', optional($attendance3GStudent)->hall) == $key ? 'selected' : '' }}>
                    {{ $text }}
                </option>
            @endforeach
        </select>
...
<div class="form-group row {{ $errors->has('attendance_mark') ? 'has-error' : '' }}">
    ...
    @foreach($students as $student)
        <label for="{{ $student -> st_regno }}" class="col-lg-3 col-md-5 text-center list-inline-item.">
            ...
            {{ in_array($student -> st_regno, old('attendance_mark', optional($attendance3GStudent)->attendance_mark) ?: []) ? 'checked' : '' }}>
            {{ $student -> st_regno }}
        </label>
    @endforeach
